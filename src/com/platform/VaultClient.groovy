...
/**
 * Vault KV v2 via Kubernetes auth (SA jenkins-kaniko).
 * Requires Jenkins plugin: pipeline-utility-steps (readJSON).
 */
...
'''
        ...
        if (!login?.auth?.client_token) {
            steps.error("Vault login missing client_token: ${loginRaw}")
        }
...
'''
        ...
        if (!secret?.data?.data) {
            steps.error("Vault secret/${secretPath} missing data: ${secretRaw}")
        }
    ...
    static Map harborCredentials(def steps, Map cfg) {
        ...
        if (!data.username || !data.password) {
            steps.error("Vault secret/${path} missing username or password")
        }
    ...
    static Map githubCredentials(def steps, Map cfg) {
        ...
        if (!user || !token) {
            steps.error("Vault secret/${path} missing username/pat")
        }
